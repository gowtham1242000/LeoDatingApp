...
//   cors: {
//     origin: "*", // Update this with your client's origin
//     methods: ["GET", "POST"]
//   }
// });

//const APP_ID = '2a501d37b13542aea209f6a418c0aa5f';
...
exports.userRequestOTP = async (req, res) => {
  try {
      ...
      if (user) {
          console.log('Existing user found:', user);
          newUser = false; // User already exists, set newUser to false
      } else {
          // If user does not exist, create a new one
          user = new User({
              mobileNumber,
              username: `user_${mobileNumber}`,
              newUser: true // Mark the new user
          });
          newUser = true; // Set newUser to true since this is a new user

          // Create a wallet for the new user with the initial coin balance
      ...
      // Send the response with the newUser flag
      console.log("newUser------", newUser);
      res.status(200).send({ message: 'OTP sent successfully', newUser });
  } catch (err) {
      console.error('An error occurred while sending the OTP:', err);
...
exports.userRequestOTP = async (req, res) => {
  try {
      ...
      // Hardcoded mobile number logic
      let otp;
      const hardcodedNumbers = ["9876543211", "8973328353"]; // Replace with actual numbers to hardcode
      if (hardcodedNumbers.includes(mobileNumber)) {
          otp = 123456; // Set hardcoded OTP
          ...
          // Extract the OTP from the response and convert it to a number
          otp = parseInt(response.data.OTP, 10); // Ensure OTP is a number
      }
...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    existingUser.userDescription = userDescription || null;
...
console.log("id-------------",id)
	...
	try 
	{
		 const languages = await Language.findById(language);
		if(!languages){
		return res.status(404).json({message:'Language not found'})
		}
		existingUser.profile.language =languages.language;
	//existingUser.profile.language = language;
	}catch(error){
		console.log("error---------",error);
		return res.status(500).json({message: 'Error fetch Language',error})
	}
...
exports.searchByUsername = async (req, res) => {
  try {
    ...
    const usersWithAvatar = users.filter(user => user.profile && user.profile.avatar);
...
exports.selectHostUserForCall = async (req, res) => {
  ...
  try {
    ...
    console.log("hostEntry0=---------------",hostEntry)
    ...
    // Check if user has enough hearts for at least one minute of call time
    ...
    // Calculate maximum call time in minutes
    ...
    res.status(200).json({
      ...
      hostDetails: {
        hostUserId: hostEntry.userId,
        hostCategory: hostEntry.category,
        hostCallType: hostEntry.callType,
        hostChannelName: hostEntry.channelName,
        token: hostEntry.token,
        appId: APP_ID
      },
      userDetails: {
        userId: userEntry.userId,
        userCategory: userEntry.category,
        userCallType: userEntry.callType,
        userChannelName: userEntry.channelName,
        maxCallTimeMinutes
      },
  ...
  try {
    ...
    res.status(200).json({
      ...
      hostDetails: {
       ...
       // token: "007eJxTYPBOnJj+IibM5bDJgl1LPETXsK/8WPs2NdvG6a/RiSVzHcoVGJINLU0MDFLMEhOTDUwSTYwtLFKNTJKNTdOSTU2A2Ohs4720hkBGhsynpYyMDBAI4rMxpCSWZOalMzAAAKzzITg=",
       // appId: APP_ID
   //    appId: "c19400d6aac04a4388e24c35fc54fc52"
        appId: "5706e283c47f4cf1be78f39d340a4c2d"
      }
    });
...
console.log("userId----------",userId)
  ...
  try {
    ...
    const users = await UserOneVsOneList.find({ category, userId: { $ne: userId } }) // $ne excludes userId
      .populate({
        path: 'userId',
        select: '-otp' // Exclude the otp field from userId
      });
...
exports.withdraw = async (req, res) => {
  try {
    ...
    if (user.profile.heartBalance < heartsToDeduct) {
    ...
    user.profile.heartBalance -= heartsToDeduct;
    user.profile.walletBalance -= amountToWithdraw;
...
exports.createCallCategory = async (req, res) => {
  try {
    ...
    // Validate input
    if (!name || !description) {
      return res.status(400).json({ message: 'Name, description, and adminId are required' });
    }
    ...
    const image = req.files.image;
    //const finalImageName = image.name.replace(/\s+/g, '_'); // Replace spaces with underscores
    const finalName =image.name.replace(/\s+/g, '_');
    const desImageDir = path.join(categoryBasePath, finalName);
    // Directory for storing category images
    /*const categoryDir = path.join(categoryBasePath, URLpathCC, name.replace(/\s+/g, '_'));
    if (!fs.existsSync(categoryDir)) {
      fs.mkdirSync(categoryDir, { recursive: true });
    }

    // Save the original image
    const imagePath = path.join(categoryDir, finalImageName);
    fs.writeFileSync(imagePath, image.data);

    // Image URL for storing in the database
    const imageURL = `https://salesman.aindriya.co.in/${URLpathCC}/${name.replace(/\s+/g, '_')}/${finalImageName}`;
...
const imagePath = path.join(desImageDir, req.files.image.name); // Construct the full image path
fs.writeFileSync(imagePath, req.files.image.data, 'binary'); // Save the image data

// Construct the image URL
const destinationImgUrl = `https://salesman.aindriya.co.in/${URLpathCC}/${finalName}/${req.files.image.name}`;
...
exports.getHostedUsers = async (req, res) => {
    try {
        ...
        const usersWithDetails = await Promise.all(hostedUsers.map(async (hostedUser) => {
            // Fetch the user details based on userId
            const user = await User.findById(hostedUser.userId).select(
                '_id username mobileNumber createdAt updatedAt profile newUser'
...
//     const uniqueChannelName = generateChannelName(hostUserId);
//     const callerToken = RtcTokenBuilder.buildTokenWithUid(
//       APP_ID,
//       APP_CERTIFICATE,
//       uniqueChannelName,
...
//     // Update the host entry with the newly generated channelName and token
//     await UserOneVsOneList.updateOne(
//       { userId: hostUserId, isHost: true },
//       { $set: { channelName: uniqueChannelName, token: callerToken } }
//     );
// io.on('connection', (socket) => {
//   console.log(`User connected with socket id: ${socket.id}`);
//     // Push the notification to the host user via socket.io
//     io.to(hostUserId).emit('hostCallDetails', {
//       token: callerToken,
//       appId: APP_ID,
//       channelName: uniqueChannelName,
//       callType: hostEntry.callType,
//     });
// })
//     // Fetch user details, heart calculations, and handle the rest as normal...

//   } catch (err) {
//     console.error('An error occurred:', err);
//     return res.status(500).json({ error: 'An error occurred while retrieving details' });
//   }
// };
...
/*
exports.selectHostUserForCallTest = async (req, res) => {
  const { userId, hostUserId, callType } = req.body;
  console.log("req.body------------", req.body);
  ...
  try {
    ...
    const hostFcmTokenEntry = await UserFcmToken.findOne({ userId: hostUserId });
    ...
    const message = {
      ...
      data: {
        ...
        callType: hostEntry.callType,
        appId: APP_ID,
        maxCallTimeMinutes: maxCallTimeMinutes.toString(), // Convert to string if needed
        joinedUserName: userEntry.username,
        joinedUserProfile: userEntry.profile.avatar,
	joinedUserFcmToken: joinedUserFcmToken.fcmToken,
      },
    };
  ...
  try {
    ...
    const message = {
      ...
      data: {
        ...
        joinedUserName: userEntry.username,
        joinedUserProfile: userEntry.profile.avatar,
        joinedUserFcmToken: joinedUserFcmToken ? joinedUserFcmToken.fcmToken : null,
      },
    };
...
//     // Send notification to followers using Firebase
//     const followerIds = followers.map(f => f.followerId);
//     sendNotificationToFollowers(followerIds, {
//       title: 'Live Stream Started!',
//       body: 'Your followed user has started a live stream.',
//       data: { channelName, hostUserId }
//     });
